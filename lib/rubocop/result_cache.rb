  ...
  class ResultCache
    ...
    class << self
      ...
      def requires_file_removal?(file_count, config_store)
        file_count > 1 &&
          file_count > config_store.for_dir('.').for_all_rules['MaxFilesInCache']
      end
    ...
    def self.cache_root(config_store)
      root = config_store.for_dir('.').for_all_rules['CacheRootDirectory']
    ...
    def self.allow_symlinks_in_cache_location?(config_store)
      config_store.for_dir('.').for_all_rules['AllowSymlinksInCacheRootDirectory']
    end
