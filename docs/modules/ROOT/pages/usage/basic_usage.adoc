...
| `   --disable-pending-rules`
| Run without pending rules.
...
| `-D/--[no-]display-rule-namess`
| Displays rule names in offense messages. Default is true.

| `   --display-only-fail-level-offenses`
| Only output offense messages at the specified `--fail-level` or above

| `   --enable-pending-rules`
| Run with pending rules.
