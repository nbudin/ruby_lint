...
Other types, such as `AllRules` / `Include` (an array), are overridden by the
child setting.
...
`['foo.rb', 'bar.rb']`. Similarly, the `AllRules:Exclude` list will contain all
...
match any pattern listed under `AllRules`/`Include` and extensionless
files with a hash-bang (`#!`) declaration containing one of the known
ruby interpreters listed under `AllRules`/`RubyInterpreters` are
inspected, unless the file also matches a pattern in
`AllRules`/`Exclude`. Hidden directories (i.e., directories whose names
...
patterns listed under `AllRules` / `Exclude` are also inspected.
If you want to apply `AllRules` / `Exclude` rules in this circumstance,
...
them under `AllRules`/`Include`. Remember that your configuration files override
...
If specified under `AllRules`, all cops are targeted.
...
If specified under a specific department, it takes precedence over `AllRules`.
