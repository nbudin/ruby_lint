...
module RuboCop
  module Rule
    module Style
      ...
      #   # good
      #   array.any?
      class SimplifyNotEmptyWithAny < Rule
...
[source,ruby]
----
describe RuboCop::Rule::Style::SimplifyNotEmptyWithAny do
...
Each cop can hold a configuration and you can refer to `rule_config` in the
...
And then on the autocorrect method, you just need to use the `rule_config` it:
...
def autocorrect(node)
  lambda do |corrector|
    internal_expression = node.children[0].children[0].source
    replacement = rule_config['ReplaceAnyWith'] || "any?"
